...
fn parse_input(input: &str) -> Vec<Row> {
    input.lines().map(|line| {
        ...
        // Multiply the string 5 times with an Unknown in between
        let mut springs_multiplied = Vec::new();
        for _ in 0..5 {
            springs_multiplied.extend(&springs);
            springs_multiplied.push(Spring::Unknown);
        }

        springs_multiplied.pop();

        // Add an OK to the start and end of the springs
        springs_multiplied.insert(0, Spring::Ok);
        springs_multiplied.push(Spring::Ok);

        let groups_part = parts.next().unwrap();

        let mut groups = groups_part
            .split(',')
            .map(|s| s.parse::<i32>().expect("Invalid number"))
            .collect::<Vec<_>>();
        
        // Multiply the groups by 5
        let mut groups_multiplied = Vec::new();
        for _ in 0..5 {
            groups_multiplied.extend(&groups);
        }


        Row { groups: groups_multiplied, springs: springs_multiplied, permutations: 1 }
    }).collect()
}
